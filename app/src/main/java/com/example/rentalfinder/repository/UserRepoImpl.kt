...
class UserRepoImpl : UserRepo{
    ...
    override fun deleteAccount(
        ...
        ref.child(userId).removeValue()
            .addOnCompleteListener {
                if(it.isSuccessful){
                    callback (true, "Account removed successfully")
                }else{
                    callback(false, "${it.exception?.message}")
                }
            }
    ...
    override fun editProfile(
        ...
        ref.child(userId).updateChildren(model.toMap())
            .addOnCompleteListener {
                if(it.isSuccessful){
                    callback (true, "Account updated successfully")
                }else{
                    callback(false, "${it.exception?.message}")
                }
            }
    ...
    override fun getUserById(
        ...
        ref.child(userId)
            .addValueEventListener(object : ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) { //snapshotma aayeko raw data lai model banyaera user ma pathairako
                    if (snapshot.exists()){
    ...
    override fun getAllUserById(callback: (Boolean, String, List<UserModel>?) -> Unit) {
